...
function Models({admin,machineId}:{admin:boolean;machineId:string}){
  ...
  {retrain&&<div className="inline-summary"><div><span>Reference candidates</span><strong>{retrain.pending??retrain.candidate_count??0}</strong></div><div><span>Per-machine target</span><strong>{retrain.batch_size_per_machine??retrain.batch_size??'—'}</strong></div><div><span>Retrain state</span><StatusBadge value={retrain.due?'DUE':'NOT DUE'} forced={retrain.due?'warning':'normal'}/></div><button className="text-button" onClick={()=>setSelected({__retrain:true,...retrain})}>How scheduling works <Icon name="info"/></button></div>}
  ...
  {selected&&<Modal title={selected.__retrain?'Retraining schedule':selected.version_id} onClose={()=>setSelected(null)} wide={!selected.__retrain}>{selected.__retrain?<div className="detail-list"><DetailRow label="Confirmed-normal candidates" value={selected.pending??selected.candidate_count??0}/><DetailRow label="Per-machine batch threshold" value={selected.batch_size_per_machine??selected.batch_size??'—'}/><DetailRow label="Currently due" value={selected.due?'Yes':'No'}/>{selected.pending_by_machine&&<Disclosure title="Candidates by machine"><pre className="code-panel compact">{JSON.stringify(selected.pending_by_machine,null,2)}</pre></Disclosure>}<p className="muted">A machine reaching the batch or age threshold starts one balanced shared-model shadow retrain. Validation must pass independently for every machine.</p></div>:<><div className="split-detail"><div className="detail-list"><DetailRow label="Status" value={selected.status} badge={tone(selected.status)}/><DetailRow label="Artifact path" value={selected.artifact_path||'—'}/><DetailRow label="Reference signature" value={selected.reference_signature||'—'}/><DetailRow label="Created" value={shortTime(selected.created_at)}/><DetailRow label="Promoted" value={shortTime(selected.promoted_at)}/><DetailRow label="Promoted by" value={selected.promoted_by||'—'}/></div><div><p className="section-label">VALIDATION REPORT</p><pre className="code-panel">{JSON.stringify(selected.validation_report||{},null,2)}</pre></div></div>{admin&&<div className="modal-actions">{selected.status!=='active'&&selected.status!=='rejected'&&<button className="primary" onClick={async()=>{await api(`/api/models/${selected.version_id}/promote`,{method:'POST'});setSelected(null);load()}}>Promote to active</button>}{selected.status!=='active'&&<button className="secondary danger-text" onClick={()=>deleteVersion(selected.version_id)}>Delete version</button>}</div>}</>}</Modal>}
...
function TrainingOptions({admin}:{admin:boolean}){
  ...
  const meta:Record<string,{label:string;desc:string;unit:string}>={
    RETRAIN_BATCH_SIZE:{label:'Retrain batch size',desc:'Confirmed-normal alerts from one machine needed before a balanced shadow retrain is due.',unit:'alerts / machine'},
    ...
    REFERENCE_WINDOW_MONTHS:{label:'Candidate window',desc:'How far back confirmed-normal candidates remain eligible for retraining.',unit:'months'},
    REFERENCE_DEDUP_WINDOW_HOURS:{label:'Dedup window',desc:'Similar candidates from the same machine within this period are treated as duplicates.',unit:'hours'},
