    ...
    def config_banner(self, title, items, fontsize_label=6.3, fontsize_value=10.0,
        ...
        if self.draw:
            ...
            for i, (label, value) in enumerate(items):
                ...
                #
                # Use FONT_HEAD (not FONT_BODY) here: this is the only place
                # in the file that renders FONT_BODY in bold, and on systems
                # where that font's bold face isn't actually installed,
                # matplotlib silently fails to draw the glyphs instead of
                # falling back — the label disappears entirely while the
                # separator lines around it still render. FONT_HEAD is bold
                # everywhere else in this report (title, values) and is
                # confirmed to render correctly, so it's the safe choice.
                label_fs = fontsize_label
                avail_w = cw_in - 0.03
                lw, _ = _text_size_in(label, label_fs, "bold", FONT_HEAD)
